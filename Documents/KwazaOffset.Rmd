...
source(paste(rdir, 'count_exceed_table.R', sep = ""))
...
ESKOMSO2PM <- read.table("Data/ESKOMSO2PM.txt", sep = "\t" ,header = TRUE)
colnames(ESKOMSO2PM)[3] <- "date"
ESKOMSO2PM$date <- dmy_hms(ESKOMSO2PM$date, tz = "Africa/Johannesburg") 
ESKOMSO2PM$yd <- yday(ESKOMSO2PM$date)
...
```{r echo=FALSE, message=FALSE}
eskom_kwa <- resample(eskom, hh_24)
eskom_kwa_base <- eskom_kwa * 0.15

# At this piont you can use setValues for populate the eskom raster with different data
eskom_kwa_base_c <- crop(eskom_kwa_base, kpext)
eskom_kwa_proj_c <- eskom_kwa_base_c * 1 / 0.15 # kry proj terug

PM <- matrix(sapply(1:364 ,function(i) rep(ESKOMSO2PM[i,1] , 84)), ncol = 364)
SO2 <- matrix(sapply(1:364 ,function(i) rep(ESKOMSO2PM[i,2] , 84)), ncol = 364)

eskom_kwa_proj_c_pm <- setValues(eskom_kwa_proj_c, PM)
names(eskom_kwa_proj_c_pm)  <- paste("pm10", ESKOMSO2PM$yd)

eskom_kwa_proj_c_so2 <- setValues(eskom_kwa_proj_c, SO2)
names(eskom_kwa_proj_c_so2)  <- paste("so2", ESKOMSO2PM$yd)

eskom_kwa_proj_c <- stack(eskom_kwa_proj_c_pm, eskom_kwa_proj_c_so2)
eskom_kwa_base_c <- eskom_kwa_proj_c * 0.15

# test
idd <- unlist(lapply(1:2, function(i) ifelse(grep(c("so2","pm10")[i], names(eskom_kwa_proj_c)), i, NA)))
a <- stackApply(eskom_kwa_proj_c, indices = idd, fun = max, na.rm = TRUE)
cellStats(a, max) == sapply(ESKOMSO2PM[,1:2], max) # test
...
```{r echo = FALSE, message=FALSE}
raster_dist_plot(hh_24, multi = c("pm10", "so2"), th=BuRdTheme, mn = "Households (baseline)", sb = "182 days") + addkwa()
```
...
```{r echo=FALSE}
raster_dist_plot(eskom_kwa_base, mn = "Secondary sulphate particulates from Hendrina power station\n in the baseline scenario in the region of KwaZamokuhle", sb = "364 days") + addkwa()
```
...
levelplot(hh_pm10_exceed, main = "Count of days when PM10 from households \nexceeded specified concentration", sub = "182 days", par.settings=BuRdTheme) + addkwa()
```
...
```{r echo = FALSE}
eskom_pm10_exceed_base = count_exceed(eskom_kwa_base_c[[grep("pm", names(eskom_kwa_base_c))]], pol = NULL, min = 0, max = 20, by = 1, knip = TRUE)

eskom_pm10_exceed_proj = count_exceed(eskom_kwa_proj_c[[grep("pm", names(eskom_kwa_proj_c))]], pol = NULL, min = 1, max = 30, by = 5, knip = TRUE)
...
```{r echo=FALSE}
eskom_so2_exceed_base = count_exceed(eskom_kwa_base_c[[grep("so2", names(eskom_kwa_base_c))]], pol = NULL, min = 0, max = 10, by = 1, knip = TRUE)

eskom_so2_exceed_proj = count_exceed(eskom_kwa_base_c[[grep("so2", names(eskom_kwa_proj_c))]], pol = NULL, min = 0, max = 10, by = 1, knip = TRUE)
...
yddE <- gsub("pm10.", "", grep("pm", names(eskom_kwa_base_c), value = T))
yddE2 <- gsub("so2.", "", grep("so2", names(eskom_kwa_base_c), value = T))
ydd <- gsub("Households_pm10_24h.", "", grep("pm", names(hh_24), value = T)) 
ydd2 <- gsub("Households_so2_24h.", "", grep("so2", names(hh_24), value = T)) 
base_PM10_vals_hh <- getValues(hh_24[[grep("pm", names(hh_24))]])
base_PM10_vals_E <- getValues(eskom_kwa_base_c[[grep("pm", names(eskom_kwa_base_c))]])[match(ydd, yddE)]
base_PM10_vals <- base_PM10_vals_E + base_PM10_vals_hh

base_SO2_vals_hh <- getValues(hh_24[[grep("so2", names(hh_24))]])
base_SO2_vals_E <- getValues(eskom_kwa_base_c[[grep("so2", names(eskom_kwa_base_c))]])[match(ydd2, yddE2)]
base_SO2_vals <- base_SO2_vals_E + base_SO2_vals_hh

# Set the values. Use the hh_24 raster as a frame
allPM10_base <- hh_24[[grep("pm", names(hh_24))]]
allPM10_base <- setValues(allPM10_base, base_PM10_vals)

allSO2_base <- hh_24[[grep("so2", names(hh_24))]]
allSO2_base <- setValues(allSO2_base, base_SO2_vals)

all_base <- stack(allPM10_base, allSO2_base)
names(all_base) <- gsub("Households_", "All_", names(all_base))

# create a people raster of the same extent as hh_24
households <- rasteriseCensus(KWA, ref = all_base, refres = c(nrow(all_base), ncol(all_base)))
...
```{r echo=FALSE, message=FALSE}
raster_dist_plot(all_base, multi = c("pm10", "so2"), mn = "Definition of the project boundary", sb = "(182 days)", th=BuRdTheme) + addkwa()
```
...
```{r echo=FALSE}
kwaza_API <- rasterAPI(all_base, verbose = TRUE)
...
rBoDPM10 <- lapply(X = rBoDPM10, FUN = function(bOutcome) {
  return(calc(x = bOutcome, fun = sum))
})
rBoDPM10 <- stack(rBoDPM10)

# tabulate total incidences per health outcome
kable(sumIncidences(rBoDPM10), digits = 2)

names(rBoDPM10) <- gsub("among.", "", names(rBoDPM10))
names(rBoDPM10) <- gsub("espiratory", "esp.", names(rBoDPM10))
names(rBoDPM10) <- gsub("hospital.admissions", "hosp.admin", names(rBoDPM10))
names(rBoDPM10) <- gsub("Cases_", "", names(rBoDPM10))
names(rBoDPM10) <- gsub("due.to.", "", names(rBoDPM10))
names(rBoDPM10) <- gsub("Cardiovascular", "CV", names(rBoDPM10))
names(rBoDPM10) <- gsub("all.ages", "all", names(rBoDPM10))
...
# summarise the number of cases across all days
rBoDSO2 <- lapply(X = rBoDSO2, FUN = function(bOutcome) {
  return(calc(x = bOutcome, fun = sum))
})
rBoDSO2 <- stack(rBoDSO2)

# tabulate total incidences per health outcome
kable(sumIncidences(rBoDSO2), digits = 2)

names(rBoDSO2) <- gsub("among.", "", names(rBoDSO2))
names(rBoDSO2) <- gsub("espiratory", "esp.", names(rBoDSO2))
names(rBoDSO2) <- gsub("hospital.admissions", "hosp.admin", names(rBoDSO2))
names(rBoDSO2) <- gsub("Cases_", "", names(rBoDSO2))
names(rBoDSO2) <- gsub("due.to.", "", names(rBoDSO2))
names(rBoDSO2) <- gsub("Cardiovascular", "CV", names(rBoDSO2))
names(rBoDSO2) <- gsub("all.ages", "all", names(rBoDSO2))
...
```{r echo = FALSE}
hh_24_proj <- hh_24 * 0.49
raster_dist_plot(hh_24_proj, mn = "Project states from households ", sb = "182 days", multi = c("pm10", "so2"), th=BuRdTheme) + addkwa()
```
...
```{r}
raster_dist_plot(eskom_kwa_proj_c, mn = "Project states from Eskom", sb = "364 days", multi = c("pm10", "so2"), th=BuRdTheme) + addkwa()
```
...
raster_dist_plot(resample(all_proj, y = raster(kpext, nrow=100, ncol = 100)), multi = c("pm10", "so2"), mn = "Project states (all sources)", sb = "182 days") + addkwa()
```
...
raster_dist_plot(resample(kwaza_API_proj,y = raster(kpext, ncol = 100, nrow=100)),
                 multi = NULL, 
...
rBoDPM10_proj <- lapply(X = rBoDPM10_proj , FUN = function(bOutcome) {
  return(calc(x = bOutcome, fun = sum))
})
rBoDPM10_proj  <- stack(rBoDPM10_proj )

names(rBoDPM10_proj) <- gsub("among.", "", names(rBoDPM10_proj))
names(rBoDPM10_proj) <- gsub("espiratory", "esp.", names(rBoDPM10_proj))
names(rBoDPM10_proj) <- gsub("hospital.admissions", "hosp.admin", names(rBoDPM10_proj))
names(rBoDPM10_proj) <- gsub("Cases_", "", names(rBoDPM10_proj))
names(rBoDPM10_proj) <- gsub("due.to.", "", names(rBoDPM10_proj))
names(rBoDPM10_proj) <- gsub("Cardiovascular", "CV", names(rBoDPM10_proj))
names(rBoDPM10_proj) <- gsub("all.ages", "all", names(rBoDPM10_proj))

levelplot(rBoDPM10_proj,
...
raster_dist_plot(PM_impact, mn = "Impact of the project activtiy on PM10 concentrations", sb = "182 days") +addkwa() + txtkwa() 
...
levelplot(rBoDPM10_impact, main = "Impact in terms of disease burden due to the \n implementation of the project activity", sub = "number of cases", par = BuRdTheme) + addkwa() + txtkwa()
```
...
```{r echo=FALSE, message=FALSE}
eskom_deficit <- eskom_kwa_base - eskom_kwa
raster_dist_plot(eskom_deficit, mn = "Deficit according to the particle precursor approach", sb = expression("\\mug/m^2"))+addkwa()
...
levelplot(crop(swi.deficit, kpext), main = "Impact deficit in the SWI approach", sub = "Standards weighted intake", margin = FALSE, par = BuRdTheme) + addkwa() + txtkwa()

levelplot(crop(swi.prop, kpext), main = "Required proporion of household SWI", sub = "Standards weighted intake", margin = FALSE, par = BuRdTheme) + addkwa() + txtkwa()
...
```{r echo=FALSE, message=FALSE}
bod_deficit <- rBoDPM10_proj <- rasterCREP(sl = endlist,
                       pollutant = "pm10", 
                       cconc = eskom_kwa, 
                       ppopr = people, 
                       bbase.conc = 10, 
...
bod_deficit <- lapply(X = bod_deficit , FUN = function(bOutcome) {
  return(calc(x = bOutcome, fun = som))
})
bod_deficit  <- stack(bod_deficit)

names(bod_deficit) <- gsub("among.", "", names(bod_deficit))
names(bod_deficit) <- gsub("espiratory", "esp.", names(bod_deficit))
names(bod_deficit) <- gsub("hospital.admissions", "hosp.admin", names(bod_deficit))
names(bod_deficit) <- gsub("Cases_", "", names(bod_deficit))
names(bod_deficit) <- gsub("due.to.", "", names(bod_deficit))
names(bod_deficit) <- gsub("Cardiovascular", "CV", names(bod_deficit))
names(bod_deficit) <- gsub("all.ages", "all", names(bod_deficit))

levelplot(bod_deficit,
          main = "Burden of disease deficit",
sub = "182 days",
par = BuRdTheme) + addkwa() + txtkwa()

bod.hh <- rBoDPM10_proj <- rasterCREP(sl = endlist,
...
##### !!!!! WAT DOEN HIERDIE EN IS DIT REG ???????? 
bod.hh <- lapply(X = bod.hh , FUN = function(bOutcome) {
  return(calc(x = bOutcome, fun = som))
})
bod.hh  <- stack(bod.hh)

names(bod.hh) <- gsub("among.", "", names(bod.hh))
names(bod.hh) <- gsub("espiratory", "esp.", names(bod.hh))
names(bod.hh) <- gsub("hospital.admissions", "hosp.admin", names(bod.hh))
names(bod.hh) <- gsub("Cases_", "", names(bod.hh))
names(bod.hh) <- gsub("due.to.", "", names(bod.hh))
names(bod.hh) <- gsub("Cardiovascular", "CV", names(bod.hh))
names(bod.hh) <- gsub("all.ages", "all", names(bod.hh))
...
cdef <- crop(bod_deficit, kpext)
levelplot(cdef, main = "Impact deficit in the BoD approach", sub = "Disease burden", margin = FALSE, par = BuRdTheme) + addkwa() + txtkwa()

levelplot(crop(bod.prop, kpext), main = "Required proporion of household BoD", sub = "Disease Burden", margin = FALSE, par = BuRdTheme) + addkwa() + txtkwa()

cellStats(bod_deficit, sum)
```
