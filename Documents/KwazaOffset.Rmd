...
```{r echo=FALSE, message=FALSE}
eskom_kwa <- resample(eskom, hh_24)[[1:182]]
...
```{r echo=FALSE}
raster_dist_plot(eskom.base, mn = "Secondary sulphate PM2.5 from Hendrina power station", sb = "364 days. Kwazamokuhle is the black dot") + addkwa2()
```
...
count_exceed_table(eskom_pm10_exceed_base, cp = "Baseline exceedence count Eskom")
count_exceed_table(eskom_pm10_exceed_proj, cp = "Project exceedence count Eskom")
```
...
```{r echo = FALSE}
raster_dist_plot(hh_24 * 0.49, mn = "Project states from households ", sb = "182 days", multi = c("pm10", "so2"), th=BuRdTheme) + addkwa()
```
...
mean(cellStats(bod.deficit, sum))
```
