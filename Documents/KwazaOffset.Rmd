...
source(paste(rdir, 'aggregateCREPstack.R', sep = ""))
...
```{r , echo = FALSE, message=FALSE}
cat("\n")
pop2sp(coal.heat, raster = TRUE, scales = list(draw = FALSE),refres = c(10,10), plot = TRUE, par.settings=BuRdTheme, resamp = TRUE, 
       main = "Approximate location of coal users for heating: \nPoint estimate with 95% CI ", 
       sub="'Yes' is coal use, 'No' is no coal use") + addkwa() + txtkwa()
cat("\n")
```
...
```{r echo=FALSE, message=FALSE}
raster_dist_plot(all_base, multi = c("pm10", "so2"), mn = "Definition of the project boundary", sb = hylabel, th=BuRdTheme) + addkwa()
```
...
```{R echo = FALSE}
raster_dist_plot(crop(combined_base, hh_24), multi = c("Households_pm10_24h\\.","Eskom_24h_pm10\\.", "All_pm10_24h\\."), meanonly = TRUE, mn = "Baseline PM contribution", sb = hylabel) + addkwa() + txtkwa()
```
...
raster_dist_plot(resample(kwaza_API_proj,y = raster(kpext, ncol = 100, nrow=100)), multi = NULL, mn = "Distribution of project API \nfor the project scenario", sb = "182 days") + addkwa() + txtkwa()
```
