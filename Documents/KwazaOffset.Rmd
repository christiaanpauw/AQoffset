...
load(paste(datadir2, "/eskom.Rda", sep = ""))
...
```{r echo=FALSE}
raster_dist_plot(eskom.base, mn = "Secondary PM2.5 from Hendrina power station", sb = "364 days. Kwazamokuhle is the black dot") + addkwa2()
```
...
prc <- c(cellStats(hh_pm10_exceed, sum) / 
                    (nrow(hh_pm10_exceed) * 
                     ncol(hh_pm10_exceed) * 
                     nlayers(hh_pm10_exceed))) *100
df <- data.frame(concentration = concLevels, ex = prc)
...
raster_dist_plot(eskom_deficit, mn = "Deficit according to the particle precursor approach")+addkwa()
...
raster_dist_plot(hh_prop_pp, mn = "Proportion of household emission require to offset Eskom emissions") + addkwa() + txtkwa()
```
...
names(bod_deficit) <- gsub("among.", "", names())
...
levelplot(crop(bod.deficit, kpext), main = "Impact deficit in the BoD approach", sub = "Disease burden", margin = FALSE, par = BuRdTheme) + addkwa() + txtkwa()
