...
pop2sp(coal.heat, 
       ...
       main = "Number of coal users for heating: \nPoint estimate with 95% CI ", 
...
\pagebreak
The modelled baseline secondary PM2.5 resulting from the eskom source is shown below.

```{r echo=FALSE}
raster_dist_plot(eskom.base) + addkwa2()
```
...
```{r echo = FALSE}
hh_pm10_exceed = count_exceed(hh_24, pol = "pm10", min = 75, max = 300, by = 50, knip = TRUE)
...
```{r echo = FALSE}
eskom_kwa <- crop(eskom, extent(hh_24))
eskom_kwa <- disaggregate(eskom_kwa, c(36,41))
eskom_kwa <- resample(eskom_kwa, hh_24, method='bilinear')

eskom_kwa_base <- crop(eskom.base, extent(hh_24))
eskom_kwa_base <- disaggregate(eskom_kwa_base, c(36,41))
eskom_kwa_base <- resample(eskom_kwa_base, hh_24, method='bilinear')
...
raster_dist_plot(kwaza_API, 
                 multi = NULL, 
                 mn = "Distribution of baseline API \nfrom households in KwaZamokuhle", 
...
```{r echo=FALSE}
raster_dist_plot(rasterAPI(crop(kwaza_eskom_24h_PM_SO2, kpext)), 
                 multi = NULL, 
                 mn = "KwaZamokuhle baseline API from households", 
                 sb = "182 days") + addkwa() + txtkwa()
```
...
```{r echo=FALSE}
levelplot(count_exceed(kwaza_API, min = 3, max = 10, by = 1, knip = TRUE) , 
          par = BuRdTheme, 
          scales=list(draw=FALSE), 
          main = "Days in KwaZamokuhle where API \nfrom households exceeded specified level", 
...
levelplot(crop(rBoDPM10_hhE, kpext),
          main = "Burden of disease for PM10 resulting from hh emissions,
...
rBoDPM10_Ee <- rasterCREP(sl = endlist,
                       pollutant = "PM10", 
                       cconc = kwaza_eskom_24h_simple[[grep(pattern = "pm10", 
                                           x = tolower(names(kwaza_eskom_24h_simple)), 
                                           fixed = TRUE)]], 
                       ppopr = people, 
                       bbase.conc = 10, 
                       rrisk.only = FALSE, 
...
levelplot(crop(rBoDPM10_Ee, kpext),
...
rBoDSO2_Ee <- rasterCREP(sl = endlist,
                       pollutant = "SO2", 
                       cconc = kwaza_eskom_24h_simple[[grep(pattern = "so2", 
                                           x = tolower(names(kwaza_eskom_24h_simple)), 
                                           fixed = TRUE)]], 
                       ppopr = people, 
                       bbase.conc = 10, 
...
levelplot(crop(rBoDSO2_Ee, kpext),
          main = "Burden of disease for SO2 
resulting from industrial point source emissions,
given as number of cases per health outcome",
sub = "364 days",
