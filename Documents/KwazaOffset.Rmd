...
KWA_R <- spTransform(KWA, CRS(wgs.proj))

addkwaR <- function() {latticeExtra::layer(sp::sp.polygons(KWA_R, lwd = 0.25))}
txtkwaR <- function() {latticeExtra::layer(panel.text(x = coordinates(KWA_R)[,1], 
                                       y = coordinates(KWA_R)[,2], 
                                       label = as.character(KWA_R@data$SP_NAME), 
                                       cex = 0.5))}
